...
def difficulties(txt, tem):
    ...
    foto.place_forget()
    framemen.destroy()
    dif0.place(y=10, relx=0.5, rely=0.1, anchor="center", relwidth=0.8)
    dif1.place(y=140, relx=0.5, rely=0.1, anchor="center", relwidth=0.8)
    dif2.place(y=270, relx=0.5, rely=0.1, anchor="center", relwidth=0.8)
    dif3.place(y=400, relx=0.5, rely=0.1, anchor="center", relwidth=0.8)
    dif4.place(y=530, relx=0.5, rely=0.1, anchor="center", relwidth=0.8)
    temma = txt
    nazvanie = tem
...
def helping(word, letters, n):
    # Проверяем, есть ли открытые буквы в слове
    global nn
...
def start_svoe():
    ...
    tema.grid(row=1, column=0, pady=60, padx=20, sticky="nw")
    ...
    frame.grid(row=2, column=0, columnspan=3, pady=20,  sticky="s")
